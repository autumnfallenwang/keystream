---
name: dev-task
description: Plan and implement the next development task. Use when the user wants to build a feature, work on a task, implement something from the design plan, or says things like "let's work on task 7", "build the send command", "what's next", or "pick up the next task".
---
...
4. Read `.claude/rules/conventions.md` for coding rules (logging levels, import conventions, error handling, naming). Follow these when writing code.
5. Identify the next task to work on — follow the phase order; earlier phases must be done before later ones.
6. If $ARGUMENTS is provided, treat it as the specific task to work on (e.g. "task 9" or "scroll-verify UI") instead of auto-detecting.
...
8. Determine which platform(s) this task targets:
   ...
   - **Pure TS core** (`src/lib/core/`): helpers for diff rendering, keymap display, config serialization. No Tauri imports, no platform code.
   - **Rust Tauri shell** (`src-tauri/src/`): `#[tauri::command]` handlers, plugin wiring, state management. Delegates to `typer-core` for anything non-trivial.
   - **Rust core library** (`typer-core/src/`): the sender, OCR verify loop, LCS alignment, chunk stitching. Platform-specific Rust (CGEvent, screen-capture piping).
   - **Swift sidecars** (`src-tauri/binaries/src/`): `ocr_helper.swift`, `region_picker.swift`. Recompile binary + commit both source and binary together.
   - Cross-cutting tasks may touch several.
9. For sender / verify work: **always test against a local TextEdit window or a synthetic captured OCR fixture first**. Real remote-VM runs are expensive (require a live VM, risk focus drift). Use `sandbox/` for live smoke tests, never commit those artifacts.
10. Consult library docs via type definitions in `node_modules/` (for TS) or `cargo doc` (for Rust). Next.js 16 in particular has breaking changes from older versions — read `node_modules/next/dist/docs/` before writing Next-specific code.
