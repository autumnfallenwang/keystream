...
# Screenshots / OCR JSON captured outside the approved folders. Approved folders:
# - docs/poc/results/       (the verified stress-run capture — the regression fixture)
# - tests/fixtures/         (synthetic captures, scrubbed)
...
if [ -n "$FOUND" ]; then
  echo "BLOCKED: Potential secrets, sandbox files, or unapproved captures in staged files:" >&2
  echo "$FOUND" | sort -u >&2
