...
Phase 3 — task 25: `check_lines(text)` Tauri command implementing the Q8 pre-send line-length check. Returns `{ ok: bool, offending: [{line: int, length: int}] }` when any source line exceeds `typer_core::config::MAX_LINE_CHARS` (80). Pure function, no side effects — no file I/O, no sidecar, no AppHandle needed. Called by the frontend's gate-strip text-check indicator (task 36). First command in the phase that's self-contained (doesn't touch region state or sidecars).
