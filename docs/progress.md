...
Phase 2 — task 18: **Integration: shift warmup regression.** Toggles `warmup_shift`; proves the recorded keystroke sequence differs and that removing warmup would cause the first-shifted-char drop documented in PoC. Covers `rules/testing.md` invariant #2. Lives in `typer-core/tests/sender_regression.rs` alongside task 17's tests; no new fixture file — test is self-contained (asserts structural properties like "first 2 events are shift-down/up" vs "not present").
