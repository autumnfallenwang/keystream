...
Phase 2 — task 9: centralise the rest of the numeric defaults into `typer-core/src/config.rs` as named `const`s. Task 8 pre-seeded `CHUNK_SIZE_LINES=5` and `CHUNK_VERIFY_SETTLE_MS=500`; task 9 adds MAX_LINE_CHARS=80, COUNTDOWN_SECS=3, EVENT_PAUSE_MS=10, MOD_HOLD_MS=10, SCROLL_SETTLE_MS=250, VERIFY_PASS_THRESHOLD=0, and wires the sender/scroll modules to read from `config` instead of their local defaults.
