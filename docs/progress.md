...
Phase 2 — task 11: copy Swift sidecar sources from [`docs/poc/ocr_helper/`](poc/ocr_helper/) to `src-tauri/binaries/src/` and compile the binaries into `src-tauri/binaries/` so Tauri's bundler can pick them up. Covers both `ocr_helper` and `region_picker`. Commit source and rebuilt binaries together (per `rules/security.md` sidecar-provenance rule).
