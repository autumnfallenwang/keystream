...
Phase 2 — task 15: **Unit: LCS alignment.** Given `sent_lines` and `seen_lines` derived from [`docs/poc/results/stress1_ocr.json`](poc/results/stress1_ocr.json), `align_lines` returns the expected aligned pairs with correct `OCR_DROP`/`OCR_XTRA` positions. Covers `rules/testing.md` invariant #4 (LCS handles drops without cascading misalignment). Lives inline in `src/align.rs`.
