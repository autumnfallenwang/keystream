...
Phase 3 — task 26: `send_with_chunked_verify(text)` command. The big event-streaming command that drives the Q7/Q9 chunked loop. Takes the user's text, splits into 5-line chunks via `chunk_text`, iterates through chunks emitting `chunk-start {index, lines}`, running `send_chunk`, sleeping `CHUNK_VERIFY_SETTLE_MS`, running `verify_visible`, emitting `chunk-pass {index}` or `chunk-fail {index, diff}`. On fail, awaits a frontend ack (`continue-after-fail` with action `skip` / `stop` / `retry-from-here` per Q10). Emits `send-complete {summary}` at the end. Complex because of the async ack handshake, shared cancel flag, and event emission — several hours of focused work. After this task the chunked send-and-verify loop works end-to-end with a frontend driving it.
