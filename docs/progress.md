...
Phase 3 — task 24: `get_region` / `clear_region` commands for UI state. These are thin pure-Rust commands on top of `typer_core::region` load/save/clear primitives — called by the frontend's gate strip (task 34) to show the "✓ Region 1707×922 at 1,108" badge and by the Clear button (task 43). No sidecar involvement; both operate on `<app_data_dir>/region.txt`.
