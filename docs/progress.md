...
Phase 2 — task 16: **Unit: chunk stitching.** Given three overlapping OCR chunks (synthetic content derived from the PoC corpus or handcrafted), `stitch_chunks` produces the expected deduplicated concatenation. Covers `rules/testing.md` invariant #3. Inline `#[cfg(test)]` in `src/stitch.rs` — task 7 smoke tests already cover 2-chunk and empty cases; task 16 adds the 3-chunk corpus-derived case.
