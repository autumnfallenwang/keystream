...
Phase 2 — task 17: **Integration: regression fixture.** Keystroke expectations for [`docs/poc/samples/code_corpus.txt`](poc/samples/code_corpus.txt) (0 skipped, 0 typing errors). `typer-core` emits the expected keystroke sequence via the trait-mocked `RecordingEventSource`; the recorded sequence is diffed against a committed fixture under `tests/fixtures/`. Covers `rules/testing.md` invariant #1 (sender accuracy). This is the biggest test-task in Phase 2 — it introduces the `tests/fixtures/` convention and a larger integration test shape.
