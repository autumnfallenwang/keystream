...
Phase 2 — task 13: first of the Phase 2 test tasks. Inline `#[cfg(test)]` in `typer-core/src/keymap.rs` asserting `char_to_keycode` returns `Some` for every printable ASCII character in the sample corpus at [`docs/poc/samples/code_corpus.txt`](poc/samples/code_corpus.txt). This pins the US-ANSI keymap table against regressions from Q1 (no unicode injection).
