...
Phase 2 — task 19 (final Phase 2 task): **Integration: chunked verify pair.** `send_chunk` + `verify_visible` against a synthetic 10-line fixture; asserts pass on clean OCR JSON and fail with expected char_diffs on a one-char-corrupted OCR JSON. The `send_chunk` side records keystrokes via the local `Recording` pattern; the `verify_visible` side uses small JSON fixture strings (clean + corrupted) passed through `parse_ocr_json` + `diff_against_tail`. Completes Phase 2's test suite. After this, Phase 2 is fully done — all 14 tasks (6–19) shipped.
