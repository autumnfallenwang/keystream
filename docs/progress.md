...
Phase 2 — task 10: thin CLI shim at `typer-core/src/bin/typer.rs` that preserves the PoC's subcommand surface (`calibrate`, `send`, `verify`, `scroll-verify`, `scroll-test`) but delegates into the new library. Enables local testing of `typer-core` without the Tauri shell. Adds `clap` to the binary's deps only (not the library).
