...
Phase 2 — task 14: **Unit: fold table.** Most of the fold-table assertions already exist from task 7's smoke tests (`folds_backtick_to_apostrophe`, `folds_angle_to_guillemet_class`, etc., in `src/fold.rs`). Task 14 formalizes the remaining documented confusion classes — double-quote variants (`"` ↔ smart quotes) and ensures each class has exactly one assertion. May be a no-op if coverage is already sufficient; in that case, review and close as done.
