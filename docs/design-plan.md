...
- **Logging hardening.** `JsonFileLogger` (`src-tauri/src/json_log.rs`) writes one JSON line per `log::*` call to `<app_data_dir>/logs/app.log`; frontend bridges through `log_info` / `log_warn` / `log_error` Tauri commands (prefixed `[webview]`). The pipeline is shipped and used everywhere, but four gaps stand: (1) no rotation policy — the file grows forever; (2) no structured `fields` object — everything is stuffed into the `message` string as `key=value`; (3) no backend-stderr / panic capture — a Rust panic prints to stderr but never reaches the JSON file; (4) no global frontend error capture — uncaught JS errors and unhandled promise rejections only land in the log if a `logErr(...)` call wraps them. Also: `open_log_dir` exists but isn't wired into any UI surface (originally planned for Settings → Help).
